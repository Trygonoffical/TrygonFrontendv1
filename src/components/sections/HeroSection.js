      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="flex justify-center mt-8 space-x-3">
          ...
          <motion.div
            ...
            animate={{ opacity: isPaused ? 1 : 0, y: isPaused ? 0 : 10 }}
            transition={{ duration: 0.3 }}
            className="absolute -top-8 left-1/2 transform -translate-x-1/2 text-xs text-white/80 bg-white/20 px-3 py-1 rounded-full backdrop-blur-sm border border-white/30"
          >
            {isPaused ? '⏸️ Paused' : ''}
          </motion.div>
          {heroSlides.map((slide, index) => (
